...
+ This code is for identifiy voluntary attrition from employees that work at \
the Frito Lay company. DDSAnalytics (my company) will deploy different \
techniques and models to generate these predictions. 
...
# Call out those that testing positive by employee #, not ID
positive_attrition$EmployeeNumber
...
```{r class.source = 'fold-show', out.width="50%"}
# Average work life balance by role 
wrklife_in_role_dist <- subset(emp_data, select=c(JobRole, WorkLifeBalance))
# Instead of creating each chart like before.. I'll create a fuction to loop
# through it 
wrklife_in_role_fun <- function(ar_df=data.frame()) {
  ar_df <- ar_df
  roles_in_data <- unique(ar_df$JobRole)  # Find the different job roles 
  
  for (role in roles_in_data) {
    tmp_df <- filter(ar_df, ar_df$JobRole==role)
    tmp_chart <- ggplot(tmp_df, aes(x=WorkLifeBalance)) +
                  geom_density() + 
                  ggtitle(role) + 
                  labs(subtitle=paste("Mean =",
                                      mean(tmp_df$WorkLifeBalance),
                                      sep=' '))
    plot(tmp_chart)
  }
}
wrklife_in_role_fun(wrklife_in_role_dist)
```
\
Sales Reps have the Highest Work Life Balance \
Research Scientist have the Lowest Work Life Balance 
